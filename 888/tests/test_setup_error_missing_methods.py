...
def test_structure_business_without_required_methods_raises_setup_error():
    ...
    assert (
        "must provide on_active_sphere_changed, load_structure_async, or load_structure"
...
def test_update_controllers_without_links_actions_raises_setup_error():
    ...
    with pytest.raises(SetupError) as exc_info:
        DatabaseEventHandler._update_controllers_with_new_db(
            window, new_db, links_actions=None
        )
...
def test_update_controllers_with_invalid_links_actions_raises_setup_error():
    ...
    assert "links_actions.links is required when switching database" in str(
        exc_info.value
    )
...
def test_links_ui_handlers_update_table_contract_error_not_suppressed():
    ...
    links_table_controller.on_links_loaded.side_effect = ValueError(
        "Contract violation"
    )
...
def test_links_ui_handlers_update_table_unexpected_error_not_suppressed():
    ...
    links_table_controller.on_links_loaded.side_effect = AttributeError(
        "Unexpected error"
    )
...
def test_resolve_structure_loader_without_methods_raises_setup_error():
    ...
    assert "must provide load_structure_async() or load_structure()" in str(
        exc_info.value
    )
