...
class Database(DatabaseBase):
    ...
    def _run_migrations(self):
        ...
        try:
            ...
            try:
                with db_lock:
                    self.connection.execute("ALTER TABLE link ADD COLUMN browser_key TEXT DEFAULT NULL")
                    self.connection.commit()
    ...
    def update_item_positions(self, table_name: str, ids_in_order: List[int]):
        ...
        if table_name not in VALID_POSITION_TABLES:
            raise ValidationError(f"Недопустимое имя таблицы для обновления позиций: {table_name}")
    ...
    def export_full_structure(self) -> Dict[str, List]:
        ...
        try:
            spheres_data = []
            spheres = self.connection.execute("SELECT * FROM sphere ORDER BY position").fetchall()

            for sphere_row in spheres:
                sphere = dict(sphere_row)
                sections_data = []
                sections = self.connection.execute(
                    "SELECT * FROM section WHERE sphere_id=? ORDER BY position", (sphere['id'],)
                ).fetchall()
                ...
                for section_row in sections:
                    ...
                    categories = self.connection.execute(
                        "SELECT * FROM category WHERE section_id=? ORDER BY position", (section['id'],)
                    ).fetchall()

                    for category_row in categories:
                        category = dict(category_row)
                        links = self.connection.execute(
                            "SELECT * FROM link WHERE category_id=? ORDER BY position", (category['id'],)
                        ).fetchall()
    ...
    def import_full_structure(self, data: List[Dict]):
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"Не удалось создать резервную копию после импорта: {backup_err}")
        except Exception as e:
        ...
        try:
            ...
            while len(files) > max_bak:
                ...
                try:
                    ...
                    logger.warning(f"Не удалось удалить старую резервную копию {old_file}: {del_err}")
        ...
        try:
            if hasattr(self.thread_local, 'conn'):
                ...
                try:
                    ...
                    logger.warning(f"Ошибка WAL checkpoint при закрытии: {checkpoint_err}")
