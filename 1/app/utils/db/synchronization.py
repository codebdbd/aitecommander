...
class EnhancedLock:
    ...
    def acquire(self, timeout: Optional[float] = None) -> bool:
        ...
        if not acquired:
            ...
            logger.warning(f"[LOCK] Таймаут захвата {self.name} потоком {thread_id} ({wait_time:.3f}s)")
            raise LockTimeout(f"Не удалось захватить блокировку {self.name} за {timeout}s")
        ...
        logger.debug(f"[LOCK] Захвачена {self.name} потоком {thread_id} (ожидание: {wait_time:.3f}s)")
        return True
    ...
    def release(self) -> None:
        ...
        if self._acquisition_time:
            ...
            if hold_time > 1.0:  # Предупреждение о длительном удержании
                logger.warning(f"[LOCK] Длительное удержание {self.name}: {hold_time:.3f}s")
...
class LockManager:
    ...
    def create_lock(self, name: str, lock_type: LockType, reentrant: bool = True) -> EnhancedLock:
    ...
    def _check_lock_order(self, thread_id: int, new_lock_type: LockType) -> None:
        ...
        try:
            current_type_indices = [self._lock_order.index(t) for t in current_lock_types]
            ...
            if any(new_type_index < idx for idx in current_type_indices):
                logger.warning(f"[LOCK] Потенциальный deadlock: попытка захвата {new_lock_type.value} "
...
class SignalGuard:
    ...
    def enter_slot(self, slot_name: str) -> bool:
        ...
        with self._lock:
            ...
            if current_count >= self._max_recursive_calls:
                logger.warning(f"[SignalGuard] Превышен лимит рекурсивных вызовов для {slot_name}: {current_count}")
                return False
            ...
            if slot_name in active_slots:
                logger.warning(f"[SignalGuard] Предотвращена рекурсия для слота: {slot_name}")
                return False
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"[SignalGuard] Вход в слот: {slot_name} (поток: {thread_id})")
    ...
    def exit_slot(self, slot_name: str) -> None:
        ...
        with self._lock:
            ...
            if logger.isEnabledFor(logging.DEBUG):
                logger.debug(f"[SignalGuard] Выход из слота: {slot_name} (поток: {thread_id})")
...
enhanced_db_lock = _lock_manager.create_lock("database", LockType.DATABASE, reentrant=True)
enhanced_tasks_lock = _lock_manager.create_lock("tasks", LockType.TASKS, reentrant=False)
...
def log_lock_stats() -> None:
    ...
    for name, stat in stats.items():
        logger.info(f"  {name}: захватов={stat.acquisition_count}, "
...
def debug_lock(lock, operation_name: str):
    ...
    with lock:
        ...
        logger.debug(f"[DEBUG_LOCK] Захвачена {operation_name} потоком {thread_id} ({acquire_time:.3f}s)")
        try:
            yield
        finally:
            hold_time = time.time() - start_time - acquire_time
            logger.debug(f"[DEBUG_LOCK] Освобождена {operation_name} потоком {thread_id} (удержание: {hold_time:.3f}s)")
