...
class BrowserProfileManager:
    ...
    def _get_cached_profiles(self, browser_key: str) -> List[Dict]:
        ...
        # Проверяем кеш
        if (browser_key in self._cache and 
            browser_key in self._last_update and
            current_time - self._last_update[browser_key] < self._cache_timeout):
    ...
    def detect_browser_from_args(self, args: str) -> Optional[str]:
        ...
        for browser_key, finder in self.finders.items():
            ...
            try:
                ...
                logger.debug(f"detect_browser_from_args: profile_data for {browser_key}={profile_data}")
                if profile_data:
                    logger.debug(f"detect_browser_from_args: detected browser_key={browser_key}")
                    return browser_key
            except Exception as e:
                logger.debug(f"Ошибка определения браузера {browser_key} из аргументов: {e}")
                continue
