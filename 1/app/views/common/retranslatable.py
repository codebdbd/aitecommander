...
class ReTranslatable:
    ...
    def __init__(
        self, *, auto_connect: bool = True, call_retranslate: bool = True
    ) -> None:
        ...
        logger.debug(
            "ReTranslatable.__init__ for %s: auto_connect=%s, call_retranslate=%s",
            widget_class,
            auto_connect,
            call_retranslate,
        )
        ...
        if auto_connect:
            logger.debug(
                "ReTranslatable: connecting %s to languageChanged signal", widget_class
            )
            self._language_service.languageChanged.connect(
        ...
        if call_retranslate and hasattr(self, "retranslateUi"):
            logger.debug(
                "ReTranslatable: calling initial retranslateUi for %s", widget_class
            )
    ...
    def _handle_language_changed(self, _lang_code: str) -> None:
        widget_class = self.__class__.__name__
        logger.debug(
            "ReTranslatable._handle_language_changed for %s: lang_code=%s",
            widget_class,
            _lang_code,
        )
        if hasattr(self, "retranslateUi"):
            ...
            logger.warning(
                "ReTranslatable: %s has no retranslateUi method!", widget_class
            )

    def _disconnect_from_language_service(self) -> None:
        try:
            self._language_service.languageChanged.disconnect(
                self._handle_language_changed
            )
