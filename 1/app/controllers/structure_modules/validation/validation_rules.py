...
class StructureDataValidator:
    ...
    def validate_sphere_create_data(
        ...
        # Required fields
        issues.extend(
            self.type_validator.validate_string(
                data.get("name"), "name", required=True, min_length=1, max_length=255
            )
        )
        issues.extend(
            self.type_validator.validate_boolean(
    ...
    def validate_section_create_data(
        self, data: dict[str, Any]
    ) -> DetailedValidationResult:
        ...
        issues.extend(
            self.type_validator.validate_integer(
    ...
    def validate_category_create_data(
        self, data: dict[str, Any]
    ) -> DetailedValidationResult:
    ...
    def validate_update_data(
        self, data: dict[str, Any], item_type: StructureItemType
    ) -> DetailedValidationResult:
