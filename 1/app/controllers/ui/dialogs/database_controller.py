...
class DatabaseController(QObject):
    ...
    operation_success = pyqtSignal(str, str)  # str, str - заголовок, сообщение об успехе
    ...
    def handle_clear_favorites(self):
        ...
        Показывает диалог подтверждения и при согласии пользователя
        отправляет сигнал для очистки избранного.
        """
        if self.dialogs.confirm_clear_favorites():
            # Отправляем сигнал - UI сам обработает очистку и обновление
            self.favorites_cleared.emit()
            self.operation_success.emit("Готово", "Избранное очищено")
    ...
    def _perform_database_restore(self, backup_path):
        ...
        if self.dialogs.confirm_database_restore(backup_path.name):
            try:
                ...
                self.operation_success.emit("Готово", f"База восстановлена из копии:\n{backup_path.name}")
        ...
        try:
            ...
            if not handle_db_error(e, self):
                ...
                try:
                    ...
                    pass
                self.operation_error.emit("Ошибка", f"Ошибка при подключении базы: {e}\nСтарая база восстановлена.")
    ...
    def handle_save_database(self):
        ...
        default_name = db_path.split("/")[-1] if "/" in db_path else db_path.split("\\")[-1]
        ...
        if save_path:
            try:
                ...
                self.operation_success.emit("Готово", f"Копия базы сохранена:\n{save_path}")
            except Exception as e:
    ...
    def handle_save_icons(self):
        ...
        if save_path:
            try:
                ...
                self.operation_success.emit("Готово", f"Архив иконок сохранён в:\n{save_path}")
            except Exception as e:
    ...
    def handle_load_icons(self):
        ...
        if zip_path:
            try:
                ...
                self.operation_success.emit("Готово", f"Иконки успешно добавлены в: {icons_dir}")
            except Exception as e:
