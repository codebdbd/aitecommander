...
class LinksMenuBuilder:
    ...
    def _add_link_item_actions(self, menu: QMenu, link: Dict) -> None:
        ...
        fav_icon = get_menu_icon('delete_favorites', self.theme) if is_favorite else get_menu_icon('add_favorites', self.theme)
    ...
    def _add_additional_actions(self, menu: QMenu, link: dict):
        ...
        undo_action_in_menu = next(
            (action for action in menu.actions() if action.text() == "&Отменить"), 
            None
        )
    ...
    def _clipboard_has_links(self) -> bool:
        ...
        try:
            ...
            if isinstance(data, dict) and 'name' in data:
                return True
            if isinstance(data, list) and any(isinstance(link, dict) and 'name' in link for link in data):
                return True
