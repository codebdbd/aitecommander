    ...
    def get_all_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'caches': {
                ...
                for cache in set(list(self._cache_hits.keys()) + list(self._cache_misses.keys()))
            },
...
def measure_time(operation_name: str, log_threshold_ms: float = 100.0) -> Callable[[F], F]:
...
def log_performance_summary() -> None:
    ...
    if stats['call_counts']:
        logger.info("\n📞 CALL COUNTS:")
        for operation, count in sorted(stats['call_counts'].items(), key=lambda x: x[1], reverse=True)[:10]:
