...
class StructureWorkerSignals(QObject):
    ...
    structure_loaded: pyqtSignal = pyqtSignal(list, int)  # List[Dict], int - структура, ID сферы
    sections_loaded: pyqtSignal = pyqtSignal(list, int)  # List[Dict], int - разделы, ID сферы
    categories_loaded: pyqtSignal = pyqtSignal(list, int)  # List[Dict], int - категории, ID раздела
    ...
    # Подсчет
    count_finished: pyqtSignal = pyqtSignal(int, list, object)  # int, List[Dict], Optional[Dict] - количество избранных
    
    # Операции CRUD
    item_created: pyqtSignal = pyqtSignal(str, int, dict)  # str, int, Dict - тип, ID родителя, данные
    item_updated: pyqtSignal = pyqtSignal(str, int, dict)  # str, int, Dict - тип, ID элемента, данные
    item_deleted: pyqtSignal = pyqtSignal(str, int, dict)  # str, int, Dict - тип, ID элемента, старые данные
...
class LinkInfoWorker(QRunnable):
    ...
    def __init__(self, link_type: str, path: str, args: str, config_module, signals: StructureWorkerSignals):
        super().__init__()
...
class LoadStructureWorker(SphereIdWorker):
    ...
    def run(self):
        ...
        try:
            self.signals.operation_started.emit(f"Загрузка структуры для сферы {self.sphere_id}...")
            ...
            with db_lock:
                ...
                all_categories = [dict(row) for row in categories_raw] if categories_raw else []
...
class LoadSectionsWorker(SphereIdWorker):
    ...
    def run(self):
        ...
        try:
            self.signals.operation_started.emit(f"Загрузка разделов для сферы {self.sphere_id}...")
            ...
            logger.debug(f"Загружено {len(sections)} разделов для сферы {self.sphere_id}")
...
class LoadCategoriesWorker(SectionIdWorker):
    ...
    def run(self):
        ...
        try:
            self.signals.operation_started.emit(f"Загрузка категорий для раздела {self.section_id}...")
            ...
            logger.debug(f"Загружено {len(categories)} категорий для раздела {self.section_id}")
...
class CreateItemWorker(BaseDbWorker):
    ...
    def __init__(self, db, item_type: str, data: Dict[str, Any], signals: StructureWorkerSignals):
        super().__init__(db, signals)
    ...
    def run(self):
        ...
        try:
            ...
            self.signals.operation_finished.emit(f"{self.item_type.capitalize()} создан")
...
class UpdateItemWorker(BaseDbWorker):
    ...
    def __init__(self, db, item_type: str, item_id: int, data: Dict[str, Any], signals: StructureWorkerSignals):
        super().__init__(db, signals)
        self.item_type = item_type  # "section" или "category"
    ...
    def run(self):
        ...
        try:
            ...
            self.signals.operation_finished.emit(f"{self.item_type.capitalize()} обновлен")
...
class DeleteItemWorker(BaseDbWorker):
    ...
    def __init__(self, db, item_type: str, item_id: int, signals: StructureWorkerSignals):
        super().__init__(db, signals)
        self.item_type = item_type  # "section" или "category"
        self.item_id = item_id
    ...
    def run(self):
        ...
        try:
            self.signals.operation_started.emit(f"Удаление {self.item_type} ID {self.item_id}...")
            ...
            self.signals.operation_finished.emit(f"{self.item_type.capitalize()} удален")
...
class CountNestedObjectsWorker(SectionIdWorker):
    ...
    def run(self):
        ...
        try:
            self.signals.operation_started.emit(f"Подсчет объектов в разделе {self.section_id}...")
            ...
            logger.debug(f"Раздел {self.section_id}: {categories_count} категорий, {links_count} ссылок")
