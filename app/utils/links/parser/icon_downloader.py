...
def save_icon(icon_url: str, domain: str, config, is_fallback: bool = False, force_refresh: bool = False) -> Optional[str]:
    ...
    if not head_response:
        ...
        if getattr(head_response, 'status_code', 0) == 304 and not force_refresh:
            ...
            if head_response.status_code in (401, 403, 405):
                logger.info(f"[icon] head_status={head_response.status_code}, trying GET fallback url={icon_url}")
            else:
                logger.info(f"[icon] skip reason=head_bad_status status={head_response.status_code} url={icon_url}")
                return None
        else:
            ct = (head_response.headers.get('Content-Type') or '').split(';')[0].strip().lower()
            if not ct.startswith('image/'):
                lower = icon_url.lower().split('?')[0].split('#')[0]
                img_ext = (lower.endswith('.png') or lower.endswith('.ico') or lower.endswith('.jpg') or
                           lower.endswith('.jpeg') or lower.endswith('.webp') or lower.endswith('.gif') or
                           lower.endswith('.bmp') or lower.endswith('.svg'))
                if img_ext:
                    logger.info(f"[icon] non_image_head ct={ct}, but URL suggests image; trying GET {icon_url}")
                else:
...
def pick_icon_parallel(soup: 'BeautifulSoup', page_url: str, domain: str, config, force_refresh: bool = False) -> Optional[str]:
    ...
    def _try_candidates_parallel(icon_urls, is_fallback: bool) -> Optional[str]:
        ...
        while True:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_map = {executor.submit(save_icon, u, domain, config, is_fallback, force_refresh): u for u in batch}
