...
class MoveOperationsHandler(TreeHandlerBase):
    ...
    def execute_move_category_command(self, category_id: int, target_id: int) -> None:
        ...
        if hasattr(main_win, "undo_stack"):
            ...
            self.logger.warning("Undo stack не найден для перемещения категории")
    
    def execute_move_links_command(self, link_ids: List[int], new_category_id: int) -> None:
        """Выполняет команду перемещения ссылок."""
        main_win = self.tree_widget.window()
        
        if hasattr(main_win, "undo_stack"):
            main_win.undo_stack.push(MoveLinksCommand(link_ids, new_category_id, main_win))
    ...
    def handle_category_section_move(self, source_id: int, parent, main_win) -> None:
        ...
        if not (isinstance(source_id, int)):
            self.logger.warning("Некорректный тип source_id для перемещения категории")
            return
        if not pdata:
            self.logger.warning("Некорректные данные целевого родителя для перемещения категории")
            return
        ...
        if hasattr(main_win, "undo_stack"):
            ...
            self.logger.warning("Undo stack не найден для перемещения категории между разделами")
    ...
    def _handle_position_update(self, source_type: str, source_id: int, parent) -> None:
        ...
        def internal_move_task():
            ...
            if not (hasattr(main_window, 'structure_business') and main_window.structure_business):
    ...
    def _prepare_position_params(self, source_type: str, source_id: int, parent) -> Dict[str, Any]:
    ...
    def _refresh_ui_after_move(self) -> None:
        ...
        if hasattr(main_win, 'structure_business') and main_win.structure_business.current_sphere_id:
