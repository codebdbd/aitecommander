...
- Icons are resolved per-type via `resolve_icon_for_link` / `resolve_link_type_icon`.
"""
...
class LinkButtonMixin:
    ...
    def _find_icon(self, icon_path: str) -> str:
        """Return icon path via the common resolver."""
        if not icon_path:
            return ""
        try:
    ...
    def _create_link_button(self, link_data: dict[str, Any]) -> QToolButton:
        ...
        try:
            resolved_path = resolve_icon_for_link(link_data)
            ...
            if not icon or getattr(icon, "isNull", lambda: True)():
                logger.warning(
                    "Icon not created/empty for link %r (path=%s)",
                    link_data.get("name"),
                    resolved_path,
                )
            button.setIcon(icon)
