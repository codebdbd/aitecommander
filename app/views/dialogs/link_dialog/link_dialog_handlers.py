...
class LinkDialogHandlers:
    ...
    def connect_signals(self) -> None:
        ...
        # Снятие подсветки ошибок при изменении соответствующих полей
        try:
            self.dialog.ui.get_widget('url_le').textChanged.connect(lambda _t: self._set_field_error('url_le', False))
            self.dialog.ui.get_widget('name_le').textChanged.connect(lambda _t: self._set_field_error('name_le', False))
            self.dialog.ui.get_widget('args_le').textChanged.connect(lambda _t: self._set_field_error('args_le', False))
            self.dialog.ui.get_widget('category_cb').currentIndexChanged.connect(lambda _i: self._set_field_error('category_cb', False))
        except Exception:
            pass
    ...
    def _on_accept(self) -> None:
        ...
        if result['is_valid']:
            ...
            if name_empty and url_empty:
                ...
                # Применяем подсветку по структурированным полям из контроллера
                problem_fields = set(result.get('problem_fields', set()))
                # Подсветка
                try:
                    self._apply_error_state(problem_fields)
                except Exception:
                    pass
                ...
                titles = [field_titles[f] for f in ['url','name','category_id','args','link_type'] if f in problem_fields]
                if titles:
                    main_msg = f"Заполните/исправьте: {', '.join(titles)}."
                    info_msg = "Проверьте выделенные поля. Полный список — в подробностях."
                else:
                    main_msg = "Пожалуйста, проверьте данные перед сохранением."
                    info_msg = "Проверьте выделенные поля и подсказки."
                ...
                # Перевести фокус на первое проблемное поле по приоритету
                try:
                    priority = ['url', 'name', 'category_id', 'args']
                    widget_map = {
                        'url': 'url_le',
                        'name': 'name_le',
                        'category_id': 'category_cb',
                        'args': 'args_le',
                    }
                    for f in priority:
                        if f in problem_fields:
                            self.dialog.ui.get_widget(widget_map[f]).setFocus()
                            break
                except Exception:
                    pass
            
    def _set_field_error(self, widget_name: str, has_error: bool) -> None:
        """Устанавливает/снимает property 'error' на виджете и реполишит его."""
        w = self.dialog.ui.get_widget(widget_name)
        if not w:
            return
        try:
            w.setProperty('error', bool(has_error))
            # Инлайн-стиль: повышаем специфичность через objectName и класс
            if has_error:
                obj = w.objectName() or widget_name
                cls = type(w).__name__
                if cls == 'QLineEdit':
                    style = f"#{obj} {{ border: 1px solid #d9534f; border-radius: 3px; }}"
                elif cls == 'QComboBox':
                    # Для комбобокса применим к самому контролу
                    style = f"QComboBox#{obj} {{ border: 1px solid #d9534f; border-radius: 3px; padding: 1px; }}"
                elif cls in ('QPlainTextEdit', 'QTextEdit'):
                    style = f"#{obj} {{ border: 1px solid #d9534f; border-radius: 3px; }}"
                else:
                    style = f"#{obj} {{ border: 1px solid #d9534f; border-radius: 3px; }}"
    ...
    def _apply_error_state(self, problem_fields: set) -> None:
        """Снимает подсветку со всех контролов и включает на проблемных."""
        field_to_widget = {
            'url': 'url_le',
            'name': 'name_le',
            'category_id': 'category_cb',
        ...
        # Снять ошибки со всех
        for wid in field_to_widget.values():
            self._set_field_error(wid, False)
        # Установить на проблемных
        for f in problem_fields:
            wid = field_to_widget.get(f)
            if wid:
                self._set_field_error(wid, True)
        # Реполиш всего диалога для гарантии применения QSS
        try:
            dlg = self.dialog
