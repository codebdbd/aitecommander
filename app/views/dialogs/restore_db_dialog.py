...
class RestoreDbDialog(BaseDialog):
    ...
    def _populate_list(self) -> None:
        ...
        try:
            ...
            self.logger.debug(f"Поиск резервных копий в: {self.backup_dir}")
            
            # Ищем файлы резервных копий
            backups = sorted(self.backup_dir.glob("links_*.db"), reverse=True)
            self.logger.debug(f"Найдено резервных копий: {len(backups)}")
            
            # Проверяем наличие файла links.db.bak
            single_backup_path = self.paths.get_db_backup_path()
            single_backup_exists = single_backup_path.exists() and single_backup_path.stat().st_size > 0
            ...
            self.logger.error(f"Ошибка при поиске резервных копий: {e}")
    ...
    def _show_no_backups_message(self) -> None:
        ...
        self.logger.info("Резервные копии не найдены")
    ...
    def _add_single_backup_item(self, backup_path: Path) -> None:
        ...
        try:
            ...
            if backup_path.stat().st_size == 0:
                self.logger.warning(f"Пустой файл резервной копии: {backup_path.name}")
                return
            ...
            self.logger.debug(f"Добавлена одиночная резервная копия: {backup_path.name}")
            
        except Exception as e:
            self.logger.warning(f"Ошибка при обработке файла {backup_path.name}: {e}")
        ...
        for backup in backups:
            try:
                ...
                if backup.stat().st_size == 0:
                    self.logger.warning(f"Пустой файл резервной копии: {backup.name}")
                    continue
                ...
                self.list_widget.addItem(label)
                self.logger.debug(f"Добавлена резервная копия: {backup.name}")
                
            except Exception as e:
                self.logger.warning(f"Ошибка при обработке файла {backup.name}: {e}")
                continue
    ...
    def _parse_datetime(self, filename: str) -> Optional[str]:
        ...
        try:
            ...
            self.logger.debug(f"Не удалось распарсить дату из имени файла: {filename}")
            return None
            
        except Exception as e:
            self.logger.debug(f"Ошибка при парсинге даты из {filename}: {e}")
            return None
    ...
    def get_selected_backup(self) -> Optional[Path]:
        ...
        try:
            ...
            if single_backup_exists:
                ...
                if row == 0:
                    self.logger.info(f"Выбрана одиночная резервная копия: {single_backup_path.name}")
                    return single_backup_path
                else:
                    # Смещаем индекс для автоматических резервных копий
                    adjusted_row = row - 1
                    if adjusted_row >= len(valid_backups):
                        self.logger.warning(f"Неверный индекс резервной копии: {row}")
                        return None
                    
                    selected = valid_backups[adjusted_row]
                    self.logger.info(f"Выбрана автоматическая резервная копия: {selected.name}")
                    return selected
            else:
                ...
                self.logger.info(f"Выбрана резервная копия: {selected.name}")
                return selected
                
        except Exception as e:
            self.logger.error(f"Ошибка при получении выбранной резервной копии: {e}")
            return None
    ...
    def _update_ok_state(self) -> None:
        ...
        ok_btn = self.buttons.button(QDialogButtonBox.StandardButton.Ok)
        enabled = (
            self.list_widget.isEnabled() and 
            self.list_widget.currentRow() >= 0 and
