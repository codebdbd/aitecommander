        ...
        try:
            ...
            if app is not None:
                ...
                if current_thread != main_thread:
                    thread_name = getattr(
                        current_thread, "objectName", lambda: "unknown"
                    )()
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            try:
                ...
                if "wrapped C/C++ object" in str(e) or "deleted" in str(e).lower():
                    logger.debug(
                        "%s: Qt object deleted during operation: %s", func.__name__, e
                    )
                    return default_return
                raise
                ...
                logger.debug(
                    "%s: AttributeError (possibly deleted object): %s", func.__name__, e
                )
                return default_return
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    if attempt < max_attempts - 1:
                        ...
                        logger.warning(
                            "%s: all %d attempts failed", func.__name__, max_attempts
                        )
