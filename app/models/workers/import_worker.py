...
class ImportStructureWorker(DatabaseWorker):
    ...
    def do_work(self, connection) -> Dict[str, int]:
        ...
        try:
            ...
            # Вставка сфер
            self.emit_progress(current, total_items, f"Импорт сфер ({len(spheres_items)})...")
            sphere_map = {}
            ...
            for ln in links_with_id + links_without_id:
                ...
                connection.execute(
                    """INSERT INTO link 
                       ...
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?)""",
                    (category_id, ln["name"], ln["url"], ln["args"], 
                     ln["type"], ln["browser_key"], ln["icon_path"], ln["position"])
                )
                current += 1
                if current % 20 == 0:
