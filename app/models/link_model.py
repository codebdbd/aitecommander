...
class LinkModel(DatabaseBase):
    ...
    def upsert_link(self, link: Dict[str, Any]) -> int:
        ...
        logger.debug(f"Upsert ссылки: {data.get('name', 'Без названия')}, browser_key={data.get('browser_key')}")
    ...
    def count_favorites(self) -> int:
        ...
        cursor = self._execute_with_error_handling(
            "SELECT COUNT(*) FROM link WHERE is_favorite=1",
            fetch_method='one'
        )
        return cursor[0]
    ...
    def get_next_position(self, category_id: int) -> int:
        ...
        try:
            ...
            logger.error(f"Ошибка получения следующей позиции для категории {category_id}: {e}")
            return 1
