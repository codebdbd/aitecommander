...
class CategoryModel(DatabaseBase):
    ...
    def insert_category(self, data: Dict[str, Any]) -> Optional[int]:
        ...
        if cursor is not None:
            ...
            logger.warning(f"Категория '{data['name']}' уже существует в разделе {data['section_id']}")
            return None
    ...
    def delete_category(self, category_id: int):
        ...
        with db_lock:
            ...
            self._execute_with_error_handling(
                "DELETE FROM link WHERE category_id=?", 
                (category_id,)
            )
    ...
    def upsert_category(self, category_data: Dict[str, Any]) -> int:
        ...
        if 'id' in category_data and category_data['id']:
            ...
            if category_id is None:
                raise ValueError(f"Категория с именем '{category_data['name']}' уже существует в этом разделе")
            return category_id
    ...
    def has_duplicate_category(self, section_id: int, category_name: str, exclude_id: Optional[int] = None):
        """Проверяет наличие дубликата категории в разделе."""
        query = "SELECT COUNT(*) as count FROM category WHERE section_id = ? AND name = ?"
