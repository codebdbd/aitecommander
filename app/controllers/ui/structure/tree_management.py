...
class TreeManagement:
    ...
    def _on_item_added(self, item_type: str, parent_id: int, data: dict) -> None:
        ...
        if item_type == "section":
            ...
            if parent_item:
                ...
                try:
                    if hasattr(self.controller, "main") and hasattr(
                        self.controller, "business"
                    ):
    ...
    def _on_item_deleted(self, item_type: str, item_id: int) -> None:
        ...
        if item:
            ...
            if next_item:
                ...
                if (
                    hasattr(self.controller, "selection_handler")
                    and self.controller.selection_handler
                ):
            ...
            if item_type == "category":
                try:
                    ...
                    if (
                        st
                        and st[0] == "section"
                        and hasattr(self.controller, "business")
                    ):
                        section_id = st[1]
                        self.controller.business.select_section(section_id)
                except Exception:
    ...
    def _create_category_item(
        self, cat_data: dict, parent_item: QTreeWidgetItem
    ) -> None:
