...
class ValidationError(Exception):
    ...
    def __init__(self, message: str, field: Optional[str] = None, item_type: Optional[str] = None):
...
class SignalEmitterProtocol(Protocol):
    ...
    def emit_signal(self, signal_type: str, item_type: str, parent_or_id: int, data: Dict[str, Any]) -> None:
        """Эмитирует сигнал."""
        ...
...
class ValidationStrategy(ABC):
    ...
    @abstractmethod
    def validate(self, data: Dict[str, Any], item_type: StructureItemType, require_parent: bool = True) -> None:
...
class StructureLogger:
    ...
    def log_operation(self, operation: str, item_type: str, item_name: str, ru_name: str) -> None:
...
class StructureSignalEmitter:
    ...
    def __init__(self, emit_signal_func: Optional[Callable[[str, str, int, Dict[str, Any]], None]] = None):
        self._emit_signal = emit_signal_func
    
    def emit(self, signal_type: str, item_type: str, parent_or_id: int, data: Dict[str, Any]) -> None:
        """Эмитирует сигнал."""
        ...
        try:
            ...
            logging.getLogger(__name__).warning(f"Ошибка эмиссии сигнала: {e}")
...
class BaseOperations:
    ...
    def __init__(
        ...
        emit_signal_func: Optional[Callable[[str, str, int, Dict[str, Any]], None]] = None,
    ...
    def _execute_with_validation(
        ...
        try:
            ...
            self.slogger.log_error(operation_name, item_type.value, f"Валидация: {e.message}")
            return None
        except StructureOperationError as e:
            self.slogger.log_error(operation_name, item_type.value, e.message)
            return None
        except Exception as e:
            self.slogger.log_error(operation_name, item_type.value, f"Неожиданная ошибка: {str(e)}")
            return None

    def _emit_signal(self, signal_type: str, item_type: str, parent_or_id: int, data: Dict[str, Any]) -> None:
    ...
    def _upsert_and_emit(
        ...
        if not ItemTypeRegistry.is_supported(item_type):
            raise StructureOperationError(
                f"Неподдерживаемый тип элемента: {item_type}",
                "upsert",
                item_type.value
            )
        ...
        try:
            result_id = self.operation_strategy.execute_upsert(self.structure_model, data_copy, config)
