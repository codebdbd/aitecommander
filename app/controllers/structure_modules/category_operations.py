...
class CategoryOperations(BaseOperations):
    ...
    def _emit_item_signal(self, signal_type: str, item_type: StructureItemType, 
                         item_id: int, data: Optional[Dict[str, Any]] = None):
        ...
        try:
            ...
            self.logger.error(f"Ошибка отправки сигнала {signal_type} для {item_type.value} {item_id}: {e}")
    
    def _validate_batch_categories(self, categories: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    def _create_item_for_import(self, item_type: str, item_data: Dict[str, Any], 
        ...
        def _create_import_operation():
            ...
            self._emit_item_signal(SignalTypes.ITEM_ADDED, enum_type, parent_id, signal_data)
            
            self.logger.info(f"Создан {item_type} для импорта: {signal_data.get('name', 'без имени')}")
            return result_id
    ...
    def _get_parent_id_for_item_type(self, item_type: str, item_data: Dict[str, Any]) -> Optional[int]:
