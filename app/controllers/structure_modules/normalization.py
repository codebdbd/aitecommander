...
def normalize_row(row: Any, logger: logging.Logger = None) -> Dict[str, Any]:
    ...
    if hasattr(row, 'keys'):
        try:
            ...
            if hasattr(keys, '__iter__'):
                ...
                active_logger.warning(f"Метод keys() объекта {type(row)} не возвращает итерируемый объект")
                return {}
        except (TypeError, ValueError, AttributeError) as e:
            active_logger.warning(f"Ошибка при обращении к keys() объекта {type(row)}: {e}")
    ...
    if hasattr(row, '__getitem__') and hasattr(row, 'keys'):
        try:
            ...
            active_logger.warning(f"Ошибка при ручном создании словаря из объекта {type(row)}: {e}")
...
def validate_normalized_data(data: Union[Dict[str, Any], List[Dict[str, Any]]], 
