...
class DatabaseInitializer:
    ...
    def _on_db_init_finished(self, result: bool, on_success: Optional[Callable] = None) -> None:
        """
        ...
        if on_success:
            try:
                ...
                logging.error("Ошибка в колбэке успешной инициализации БД: %s", e, exc_info=True)
    
    def _on_db_init_error(self, error: Exception, on_error: Optional[Callable[[Exception], None]] = None) -> None:
        """
        ...
        """
        logging.error("Ошибка инициализации БД в фоне: %s", error, exc_info=True)
        ...
        if on_error:
            try:
                ...
                logging.error("Ошибка в колбэке ошибки инициализации БД: %s", e, exc_info=True)
