...
class PositioningOperations(BaseOperations):
    ...
    def __init__(self, structure_model, logger: logging.Logger, execute_with_error_handling):
    ...
    def update_item_positions(self, table_name: str, ids_in_order: List[int]) -> bool:
        ...
        if validation_error:
            self.logger.warning(f"Ошибка валидации при обновлении позиций: {validation_error}")
            return False
        ...
        def _update_positions_operation():
            ...
            if hasattr(self.structure_model, 'validate_ids_exist'):
                if not self.structure_model.validate_ids_exist(table_name, ids_in_order):
                    self.logger.warning(
        ...
        if result:
            self.logger.debug(f"update_item_positions завершен успешно для таблицы '{table_name}'")
        else:
            self.logger.error(f"update_item_positions завершился с ошибкой для таблицы '{table_name}'")
        
        return result if result is not None else False
    
    def _validate_positioning_params(self, table_name: str, ids_in_order: List[int]) -> Optional[str]:
        ...
        if len(set(ids_in_order)) != len(ids_in_order):
            duplicates = [id_val for id_val in set(ids_in_order) if ids_in_order.count(id_val) > 1]
    ...
    def batch_update_positions(self, updates: List[UpdateSpec]) -> bool:
        ...
        self.logger.info(f"Начинается пакетное обновление позиций для {len(updates)} таблиц")
        ...
        for i, update_data in enumerate(updates):
            if not isinstance(update_data, tuple) or len(update_data) != 2:
                self.logger.error(f"Некорректный формат данных в позиции {i}: ожидается кортеж (table_name, ids)")
                continue
            ...
            if self.update_item_positions(table_name, ids_in_order):
                ...
                self.logger.error(f"Ошибка при обновлении позиций для таблицы '{table_name}'")
