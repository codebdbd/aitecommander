...
class ShutdownHandler:
    ...
    def __init__(self, name: str, handler: Callable, priority: ShutdownPriority, 
                 timeout: int = None, critical: bool = False):
        self.name = name
...
class AppShutdownController:
    ...
    def perform_shutdown(self, event):
        ...
        with self._shutdown_lock:
            if self.shutdown_in_progress:
                logging.warning("Shutdown already in progress, ignoring duplicate request")
                return
    ...
    def _execute_shutdown_sequence(self):
        ...
        for priority in ShutdownPriority:
            ...
            if remaining is not None and remaining <= 0:
                logging.error("Global shutdown deadline exceeded before priority %s", priority.name)
                break

            handlers = handlers_by_priority[priority]
            logging.debug(f"Executing shutdown priority {priority.name} with {len(handlers)} handlers (remaining ~{remaining} ms)")

            try:
                if self.parallel_execution and len(handlers) > 1 and priority != ShutdownPriority.CRITICAL:
                ...
                logging.error(f"Error in priority {priority.name}: {exc}", exc_info=True)
                if priority == ShutdownPriority.CRITICAL:
                    raise

    def _group_handlers_by_priority(self) -> Dict[ShutdownPriority, List[ShutdownHandler]]:
    ...
    def _execute_handlers_sequential(self, handlers: List[ShutdownHandler], remaining_ms: int | None = None):
        ...
        for handler in handlers:
            ...
            if rem is not None and rem <= 0:
                logging.error("Global shutdown deadline exceeded during sequential handlers")
                break
            eff_timeout = min(handler.timeout, rem) if rem is not None else handler.timeout
            self._execute_single_handler(handler, override_timeout_ms=eff_timeout)

    def _execute_handlers_parallel(self, handlers: List[ShutdownHandler], remaining_ms: int | None = None):
        ...
        eff_ms = min(max_handler_timeout + 1000, remaining_ms) if remaining_ms is not None else (max_handler_timeout + 1000)
        timeout_seconds = (eff_ms / 1000.0) if eff_ms is not None else None

        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            future_to_handler = {
                executor.submit(self._execute_single_handler, handler, min(handler.timeout, remaining_ms) if remaining_ms is not None else handler.timeout): handler
                for handler in handlers
            }

            try:
                # Если timeout_seconds None, ждем без таймаута
                iterator = as_completed(future_to_handler, timeout=timeout_seconds) if timeout_seconds is not None else as_completed(future_to_handler)
    ...
    def _timeout_context(self, timeout_ms: int, handler_name: str):
        ...
        try:
            yield
            if timeout_occurred:
                raise ShutdownTimeoutError(f"Handler '{handler_name}' timed out after {timeout_seconds}s")
        finally:
            if timer:
                timer.cancel()

    def _execute_single_handler(self, handler: ShutdownHandler, override_timeout_ms: int | None = None):
        ...
        try:
            ...
            eff_timeout = override_timeout_ms if override_timeout_ms is not None else handler.timeout
    ...
    def add_shutdown_handler(self, name: str, handler: Callable, 
        ...
        logging.debug(f"Registered shutdown handler: {name} (priority: {priority.name})")
    ...
    def _shutdown_controllers(self):
        ...
        for attr_name, display_name in controllers_to_shutdown:
            try:
                ...
                logging.error(f"Error shutting down {display_name}: {exc}", exc_info=True)
    ...
    def _wait_for_thread_pools(self):
        ...
        try:
            ...
            if pool and pool.activeThreadCount() > 0:
                logging.debug(f"Waiting for {pool.activeThreadCount()} global threads to finish")
                if not pool.waitForDone(timeout):
                    logging.warning("Global thread pool did not finish within timeout, forcing cleanup")
        ...
        try:
            if hasattr(self.window, "thread_pool"):
                ...
                if local_pool and local_pool.activeThreadCount() > 0:
                    logging.debug(f"Waiting for {local_pool.activeThreadCount()} local threads to finish")
                    if not local_pool.waitForDone(timeout):
                        logging.warning("Local thread pool did not finish within timeout, forcing cleanup")
                        try:
                            local_pool.clear()
                        except Exception as clear_exc:
                            logging.error(f"Error clearing local thread pool: {clear_exc}")
        except Exception as exc:
