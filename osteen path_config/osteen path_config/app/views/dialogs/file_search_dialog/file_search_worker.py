    ...
    def _perform_search(self):
        ...
        for root, dirs, files in os.walk(search_path):
            ...
            for file in files:
                ...
                if match_result:
                    found_content = match_result if isinstance(match_result, str) else ""
    ...
    def _matches_criteria(self, file_path, filename):
        ...
        try:
            ...
            if self.search_params.get('name_mask'):
                if not fnmatch.fnmatch(filename.lower(), self.search_params['name_mask'].lower()):
                    return False
            
            # Проверка регулярного выражения для имени
            if self.search_params.get('name_regex'):
                pattern = self.search_params['name_regex']
                flags = 0 if self.search_params.get('case_sensitive', False) else re.IGNORECASE
                if not re.search(pattern, filename, flags):
                    return False
            
            # Проверка размера файла
            if self.search_params.get('min_size') is not None or self.search_params.get('max_size') is not None:
                try:
            ...
            for encoding in encodings:
                try:
                    ...
                    if is_regex:
                        ...
                        search_text = content_text if case_sensitive else content_text.lower()
