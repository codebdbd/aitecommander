...
class FileSearchWorker(QRunnable):
        ...
        try:
            ...
            for root, dirs, files in os.walk(self.config['root']):
                ...
                for filename in files:
                    ...
                    if _matches_common(self.config, filepath, filename, name_regex, content_regex):
    ...
    def _matches_criteria(self, filepath: str, filename: str, name_regex, content_regex) -> bool:
        """DEPRECATED: вызывать _matches_common напрямую из run()."""
        return _matches_common(self.config, filepath, filename, name_regex, content_regex)
