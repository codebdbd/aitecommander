...
def test_get_links_fields_whitelist_filters_invalid_and_logs(
    db_in_memory: Database, caplog
):
    cat_id, _ = _seed_one_link(db_in_memory)

    with caplog.at_level("WARNING"):
        rows = db_in_memory.links.get_links(
            cat_id,
            fields=[
                "id",
                "category_id",
    ...
    assert any(
        "игнорированы недопустимые поля" in rec.message for rec in caplog.records
    )
...
def test_get_links_empty_fields_fallbacks_to_default(db_in_memory: Database):
    ...
    rows = db_in_memory.links.get_links(
        cat_id, fields=["__x__", "__y__"], all_fields=False
    )
    assert isinstance(rows, list) and len(rows) == 1
    row = rows[0]
