...
class TestImportFullStructureFixes(unittest.TestCase):
    ...
    def test_input_data_immutability(self):
        ...
        with (
            patch(
                "app.models.db.Database.connection",
            ...
            self.assertIn(
                "categories",
                self.test_data[0]["sections"][0],
                "Ключ 'categories' был удален из исходных данных",
            )
            self.assertIn(
                "links",
                self.test_data[0]["sections"][0]["categories"][0],
    ...
    def test_thread_safety_with_db_lock(self):
        ...
        with (
            ...
            self.assertTrue(
                lock_acquired, "db_lock не был использован в import_full_structure"
            )
    ...
    def test_concurrent_access_safety(self):
        ...
        self.assertEqual(
            len(errors), 0, f"Ошибки при параллельном выполнении: {errors}"
        )
        self.assertEqual(
            len(results),
            num_workers,
    ...
    def test_deepcopy_usage(self):
        ...
        with (
            patch("app.models.db.copy.deepcopy") as mock_deepcopy,
            patch(
                "app.models.db.Database.connection",
                new_callable=PropertyMock,
                return_value=mock_conn,
