...
def test_make_icon_result_not_found(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(
        iu.icon_path_service, "get_user_icons_dir", lambda: tmp_path / "user_icons"
    )
    monkeypatch.setattr(
        iu.icon_path_service, "get_ui_icons_dir", lambda: tmp_path / "ui_icons"
    )
...
def test_make_icon_result_permission_denied_on_exists(
