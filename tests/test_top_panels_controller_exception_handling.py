...
class TestTopPanelsControllerExceptionHandling(unittest.TestCase):
    ...
    def test_refresh_favorites_business_error_logged_and_not_raised(self):
        ...
        ctrl = TopPanelsController(Mock(), fav_widget=fav, recent_links_widget=rec, links_business=links_business)

        with patch('app.controllers.ui.top_panels_controller.logger.exception') as mock_exc:
    ...
    def test_request_recents_refresh_unexpected_error_is_raised(self):
        fav = DummyFavWidget()
        rec = DummyRecentsWidget()
        ctrl = TopPanelsController(Mock(), fav_widget=fav, recent_links_widget=rec, links_business=Mock())

        with patch.object(ctrl._recent_refresh_timer, 'start', side_effect=RuntimeError('timer fail')):
            with patch('app.controllers.ui.top_panels_controller.logger.exception') as mock_exc:
