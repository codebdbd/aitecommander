...
def test_atexit_handlers_do_not_accumulate_on_recreate(monkeypatch):
    ...
    # 1st create
    ic.find_favicon_candidates(soup, base_url, config=types.SimpleNamespace(), on_manifest_icons=lambda urls: None)
    ...
    # 2nd create
    ic.find_favicon_candidates(soup, base_url, config=types.SimpleNamespace(), on_manifest_icons=lambda urls: None)
    assert len(registered) == 1, "Exactly one handler should be registered after recreation"
